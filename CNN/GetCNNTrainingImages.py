...
def main():
    ...
    while(True):
        ...
        im_cvrs = cv2.resize(im_cv, None, fx=ReshapeRatio, fy=ReshapeRatio, interpolation=cv2.INTER_CUBIC)

        Num = Num+1
        Name = str(int(piece_x*ReshapeRatio))+'_'\
             + str(int(piece_y*ReshapeRatio))+'_'\
             + str(int(board_x*ReshapeRatio))+'_'\
             + str(int(board_y*ReshapeRatio))+'_'\
             +str(Num)+".png"
        Path ='/home/zhy/Pic/'
        cv2.imwrite(Path+Name, im_cvrs, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
        cv2.circle(im_cvrs, (int(piece_x*ReshapeRatio), int(piece_y*ReshapeRatio)), 3, (0, 0, 255), -1)
        cv2.circle(im_cvrs, (int(board_x*ReshapeRatio), int(board_y*ReshapeRatio)), 3, (255, 0, 0), -1)

        cv2.imshow('Image',im_cvrs)
        cv2.waitKey(5)
